...
# Import simulation builders from the new unsteady core file
...
if __name__ == "__main__":
    ...
    # Stroke frequency (e.g., 1 full stroke per time unit)
    omega = 2.0 * jnp.pi
    ...
    # Constant dipole strength is fixed at 1.25
    p_const_val = 1.25
    
    # Volumetric fractions: 10 values from 0.1 to 0.5
    vol_fractions = np.linspace(0.1, 0.5, 10)
    
    # Dipole amplitude: 10 values logarithmically spaced from 0.1*1.25 to 10*1.25
    amp_vals = np.logspace(np.log10(p_const_val), np.log10(100.0 * p_const_val), 10)

    # Map the 1D job index to the 2D parameter grid
    phi_idx = job_index // 10
    amp_idx = job_index % 10

    phi = vol_fractions[phi_idx]
    p_amp_val = amp_vals[amp_idx]

    # 3. Setup specifically for this run
    print("\n" + "-"*60)
    print(f"ARRAY JOB {job_index} | phi = {phi:.3f} | p_amp = {p_amp_val:.3f}")
    print("-"*60)
    ...
    sigma_amp = jnp.ones(N) * p_amp_val
    ...
    sim_fn = build_trajectory_generator(
        Lx, Ly, dt, solve_flow_fn, sigma_const, sigma_amp, omega, phases, v0=v0, Dr=Dr
    )
    ...
    output_filename = f"swimmer_traj_unsteady_phi_{phi:.3f}_amp_{p_amp_val:.3f}.h5"
