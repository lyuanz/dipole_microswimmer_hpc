class AxisymmetricSwimmers2D:
    def __init__(self, positions, angles, dipole_const, dipole_amp, phases):
        """
        ...
        self.dipole_const = jnp.asarray(dipole_const, dtype=jnp.float32)
        self.dipole_amp = jnp.asarray(dipole_amp, dtype=jnp.float32)
        self.phases = jnp.asarray(phases, dtype=jnp.float32)
...
def build_quasi2d_stokes_solver(Lx, Ly, Lz, Nx, Ny, Nz, mu=1.0, eps=0.5, v0=0.5):
    ...
    # multiplier is exp(-81), which is mathematically zero for float64.
    ...
    @jax.jit
    def solve_flow(positions, angles, sigmas_active, sigmas_rep, neighbor_idx):
        """
        ...
        # ---------------------------------------------------------------------
        # Use sigmas_active for Active Pumping
        px, py = jnp.cos(angles), jnp.sin(angles)
        M_xx = sigmas_active * px * px
        M_xy = sigmas_active * px * py
        M_yy = sigmas_active * py * py
...
def build_trajectory_generator(Lx, Ly, dt, solve_flow_fn, sigma_const, sigma_amp, omega, phases, v0=0.5, Dr=0.01):
    """
    ...
    def wrapper_simulation(initial_positions, initial_angles, nbrs_init, num_steps, prng_key):
        ...
        def scan_step(carry, _):
            ...
            current_sigmas = sigma_const + sigma_amp * jnp.sin(omega * t + phases)
            
            # 1. Update Neighbor List
            nbrs = nbrs.update(pos)
            
            # 2. Extract sparse neighbor indices and feed them to the solver
            u_fluid, omega_fluid, _ = solve_flow_fn(pos, current_angles, current_sigmas, sigma_const, nbrs.idx)
